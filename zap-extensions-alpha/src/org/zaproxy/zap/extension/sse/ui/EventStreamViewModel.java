...
/**
 * This model uses a database table to load only needed entries from database.
 * Moreover it shows only those entries that are not blacklisted by the given
 * filter.
 */
		...
		try {
			synchronized (cachedRowCountSemaphore) {
				if (cachedRowCount == null) {					
					cachedRowCount = table.getEventCount(getCriterionMessage(), getCriterianInScope());
				}
...
//	}
	 ...
	 * <p>
	 * Retrieves Server-Sent Events from database with full payload.
	 * </p>
	 * 
	 * @param rowIndex
	...
	public void fireMessageArrived(ServerSentEvent event) {
		boolean isWhitelistedChannel = (activeStreamId == null) || event.getStreamId().equals(activeStreamId);
